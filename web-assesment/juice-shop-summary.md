...
Parameterized queries / prepared statements must replace all dynamic SQL construction to prevent injection.

Input validation and output encoding should be enforced on all user-supplied data to prevent XSS.

Enforce access controls server-side — never rely on client-side URL restrictions for admin areas.

Remove or restrict the /ftp directory and ensure sensitive files are never publicly accessible.

Implement a Web Application Firewall (WAF) as a defence-in-depth measure.
Conduct regular DAST/SAST scanning and penetration testing as part of the SDLC.
